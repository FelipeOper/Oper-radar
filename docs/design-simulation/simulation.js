...
(() => {
  ...
  const common = {
    ...
    'veiculo.html': ['Detalhe do veículo','Volvo FH 540 · 2021 · AAA0A00', `
      ${card('Recorte do veículo','Minha Loja · Curitiba/PR · caminhão pesado · 30 dias',`<span class="or-badge or-badge--neutral">Placa fictícia</span><p>Anúncio próprio fictício · R$ 515.000 · 312.000 km · 2021.</p>`)}
    ...
    'anuncio.html': ['Oferta contextual','Volvo FH 540 · 2021 · EX-001', `
      ${card('Anúncio fictício','Rota Exemplo Caminhões · Curitiba/PR · placa CCC0C00',`<span class="or-badge or-badge--neutral">Ativo no radar</span><p>Preço atual R$ 489.900 · anterior R$ 499.900 em 21/09/2026. Histórico ilustrativo.</p>`)}
    ...
    'conta.html': ['Minha conta','Identidade de demonstração', `
      ${card('Conta de exemplo','Nenhuma credencial real ou sessão ativa',`<p>Usuário: Pessoa Exemplo · papel: demonstração · e-mail: exemplo@invalid.example.</p><span class="or-badge or-badge--neutral">Conta de demonstração</span>`)}
  ...
  const shell = (title, subtitle, body, includeIntro = true) => `<div style="display:flex;min-height:100vh;overflow-x:hidden">${sidebar()}<div style="flex:1;min-width:0"><header class="or-topbar"><div class="or-topbar__title"><span class="or-topbar__crumb">OPER RADAR / DEMONSTRAÇÃO</span><h1 class="or-topbar__h">${title}</h1></div><div class="or-topbar__actions"><button class="or-btn or-btn--secondary" type="button" data-analyst>Analista IA</button></div></header><main class="main" style="max-width:1440px;margin:auto;padding:32px var(--gutter) 80px">${includeIntro ? `<section class="or-card simulation-intro"><span class="or-sectiontag or-sectiontag--accent">DEMONSTRAÇÃO INTERATIVA</span><h2>${subtitle}</h2><p>Dados inteiramente fictícios · nenhuma conexão com produção.</p><p>${link('index.html','Mapa de telas')} ${link('configuracoes.html','Configurações')} ${link('conta.html','Conta')}</p></section>` : ''}${body}${limits}</main></div></div>${mobileNav()}`;
  if (common[here]) {
    ...
    document.querySelector('#root .simulation-intro')?.remove();
    enhancePreserved();
  }
  ...
  const adaptOfficialNavigation = () => {
    ...
    if (introCard) { introCard.style.padding = compact ? '8px 12px' : ''; introCard.style.marginBottom = compact ? 'var(--space-3)' : ''; const heading = introCard.querySelector('h2'); if (heading) heading.style.margin = compact ? '4px 0' : ''; const description = introCard.querySelector('p:first-of-type'); if (description) description.style.display = compact ? 'none' : ''; const links = introCard.querySelector('p:last-child'); if (links) links.style.display = compact ? 'none' : ''; }
