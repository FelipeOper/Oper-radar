...
- **Achado aberto (herdado da `main`, não é regressão deste PR):** tipos reais fora do mapa de categorias caem em "Outros" no backend (`oper-radar-api/lib/market_taxonomy.php`) e no frontend (`app/src/marketTaxonomy.js`): `Carreta` (3.787 anúncios ativos), `Implementos-agricolas` (4.893), `Aviao`, `Sementes`, `Trator-skidder`, `Manipulador-telescopico`. Efeito: o mercado "Caminhões e implementos" soma 12.236 anúncios (Caminhao 11.778 + Carroceria-sobre-chassi 453 + Trailer 5) e **não inclui as carretas**; "Implementos rodoviários" aparece com 157 revendas e "Outros" com 912. Decisão de produto pendente: classificar `Carreta` como implemento (e `Implementos-agricolas` como agrícola). Não foi alterado.
