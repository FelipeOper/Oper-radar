...
- **O que é**: decisão (c) do Felipe. Por UF, os modelos (marca + modelo + ano) com melhor índice regional (`lib/regional_modelo.php`, já usado na Minha Loja e em Detalhes do modelo) e, em cada um, os anúncios candidatos da UF com nota 0–100 explicável: preço vs mediana da UF 35, vs FIPE 20 (só ano ≥ 2006, caminhão, cavalo/chassi/vazio e vínculo `alto`; sem isso o peso é redistribuído), sinal de negociação 20 (redução de preço em 30 dias e tempo no radar; sinal, não prova), liquidez do modelo na UF 15, qualidade 10. Só grupos com 10+ anúncios no Brasil e praças publicáveis (5+ preços válidos e histórico de eventos). Linguagem: "candidatos à negociação", nunca "ideal". Tela: seção no topo de Oportunidades, abas por UF, "Por que esta nota".
- **Ressalvas honestas**: sem MySQL local, as 3 consultas novas (anúncios ativos, reduções 30 d, saídas 180 d com subconsulta correlacionada) **não foram medidas**; o cache de 10 min protege as chamadas seguintes, mas a primeira pode ser lenta. Conferir o tempo da primeira chamada ao vivo (limite de 60 s no PHP). Universo desta versão: caminhões sem implemento; implementos rodoviários ficam para depois (pergunta 2 da spec segue em aberto, assumido Pesado).
- Zips em `/home1/pro93061/backups/` (`Downloads\OperRadar-Release2.1`, mesma pasta): API `oper-radar-api-release1.7-1cbef99.zip` `07db71707461e3b56c41f1a41c06fbc556df1059a9f328d70475fb8ad46b820e` (`oportunidades_compra.php` `18510291…`, `lib/oportunidade_compra.php` `b01f697a…`); beta `d764b7e1b7b3ce2586b07c8663caa817154feff7e0eb85398ee206ac6216ec70` (`index.html` `57360710…`, `index-XkOHSGJi.js` `b0c05df2…`); produção `bc622fe99199ff634120aac211163584749bbb47f745c77809ba4a2f1a1afcb5` (`index.html` `e586d7d2…`, `index-BQj9K_Kh.js` `757f4d56…`). Pré-checagens: `lib/market_quality.php` `e90ccf6e…` (Release 2.3), `lib/regional_modelo.php` e `lib/store_market.php` presentes, `index.html` do beta `a3541a4e…` e da produção `b1f8e21c…` (Release 2.5).
- **Publicado em 26/09/2026** (comando colado por Felipe no Terminal do cPanel; a saída do Terminal, com nomes dos backups `api-release1.7-*` e `oper-radar-frontend-release2.6-*` e os marcadores `API_OK`/`BETA_OK`/`PROD_OK`, não foi enviada ao Claude: os arquivos foram conferidos ao vivo, não pelo log). Conferido ao vivo sem login: produção serve `index-BQj9K_Kh.js` (200, 264.924 bytes) e `index.html` com sha256 `e586d7d2…` (igual ao pacote); beta serve `index-XkOHSGJi.js`; `oportunidades_compra.php` responde 401 sem sessão (era 404). **Conferido ao vivo autenticado:** a primeira chamada da tela Oportunidades levou **955 ms** (status 200, 51 KB), muito abaixo do limite de 60 s; chamadas seguintes com cache de 10 min (`em_cache: true`) levam ~115–480 ms. Resposta: 7 UFs (GO, MG, MT, PR, RS, SC, SP) com 3 modelos cada e 105 anúncios candidatos; nenhum `anuncio_id` repetido, todas as URLs http/https, soma dos componentes do índice bate com a pontuação em todos os 21 modelos. Rotas irmãs seguem 200 (`hoje_stats`, `mercado_painel`, `kpis`, `insights`, `lojistas`, `minha_loja`); único erro de console é o ruído de extensão do Chrome (`Could not establish connection`). **Observação (não é bug comprovado):** 11 grupos de anúncios distintos (ids e URLs de origem diferentes) repetem título, preço e revenda, por exemplo 4 × VOLVO FH 540 2023/2024 a R$ 810.000 da Comelli (GO) e 4 × MB 1635 2017/2018 a R$ 253.500 da Marka (SP); podem ser unidades idênticas no estoque ou anúncios duplicados no marketplace, e a tela os lista como candidatos separados. Decidir se agrupa. Não testado: os botões "Criar ação" e "Por que esta nota" com dados reais, e o mobile.
- Reversão: apagar `oportunidades_compra.php` e `lib/oportunidade_compra.php` (a tela mostra erro amigável sem eles) ou restaurar de `api-release1.7-*`; restaurar `index.html` de `oper-radar-frontend-release2.6-*`.

## Release 2.5 — frontend (beta e produção): Comparador no layout da DEMO + ajustes da Minha Loja — 26/09/2026 (PUBLICADO; bundle conferido, verificação autenticada pendente de login)
...
- **Publicado em 26/09/2026** (comando colado por Felipe no Terminal do cPanel). Conferido ao vivo sem login: `index.html` de produção referencia `index-XnIIfpsf.js`. Conferência autenticada do Comparador com dados reais: pendente (sessão expirada, 401).
...
- **Publicado em 25/09/2026 ~10h40** (extração por Felipe no Terminal do cPanel; upload dos zips por Claude). Saída: `BETA_PRECHECK_OK`/`BETA_OK` (`index.html` `03cecfb7…`, `index-BjEvlm97.js` `5d886de0…`); `PROD_PRECHECK_OK`, `.htaccess` mantido (`14c69284…`), backup `/home1/pro93061/backups/oper-radar-frontend-release2.4-20260925-104028`, `REFS_NO_INDEX: 4`, `index.html` `780cd403…`, `index-kfmlcEva.js` `463aff9f…`, `PROD_OK`. Hashes finais iguais ao pacote. Conferido ao vivo (sem login): `index.html` de produção referencia `index-kfmlcEva.js`. **Conferido ao vivo em produção (autenticado):** 77 veículos (49 ativos, 28 vendidos) → 49 cartões ativos coerentes com o resumo: 0 acima do mercado, 5 competitivos, 44 sem comparação. Dos 44: 28 sem vínculo FIPE (todos vindos do XML) e 16 com FIPE mas 0–2 anúncios equivalentes no mercado — é limitação do dado, não da tela. Evidência do cartão mostra mercado nacional, amostra (5 de 5) e confiança; painel do veículo abre com abas Cadastro, Mercado nacional (+3,4% vs mediana, igual ao cartão) e Regiões; sem erro de tela. Não testei alterar status/excluir para não escrever nos dados reais. Melhorias anotadas para o próximo release: alerta separando "sem vínculo FIPE" de "poucos anúncios", e vendidos por último na ordenação por posição.
