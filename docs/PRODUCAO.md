...
- **Publicada em 24/09/2026** (extração por Felipe no Terminal do cPanel; upload dos zips por Claude). O Felipe informou que rodou os dois comandos; a saída do terminal (backup `api-release1.3-…`) não foi colada, então o backup não está registrado aqui — localizar em `/home1/pro93061/backups/api-release1.3-*` se precisar reverter. Conferido ao vivo (autenticado, sem erro de console): `index.html` do beta = `30906009…` e bundle `index-C2m3WyHa.js` (batem com o manifesto); o comportamento novo só existe com a API 1.3 no ar.
