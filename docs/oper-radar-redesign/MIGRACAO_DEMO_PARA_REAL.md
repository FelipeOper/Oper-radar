...
| Reduções de preço | tabela `mudanca_preco` | Dado existe; falta endpoint agregado por período/UF/revenda |
| Idade do estoque / tempo no ar | `anuncio.primeira_vez_visto`, `anuncio_snapshot.dias_no_ar` | Dado existe; falta agregação |
| Saídas observadas (2ª confirmação) | `anuncio.status`, `data_remocao`, `venda_estimada` | Regra de 2 confirmações: **a verificar** no coletor |
| Frescor da coleta por UF | `execucao_coleta` (janelas 07h/19h) | Dado existe; falta endpoint "coleta atrasada por UF" |
...
2. `reducoes_preco.php` — reduções por período/UF/revenda/modelo a partir de `mudanca_preco`.
3. `frescor_coleta.php` — última coleta completa por UF a partir de `execucao_coleta`.
4. Extensão de `hoje_stats.php` — feed de movimento (novos, reduções, saídas) com evidência.
