...
$fipeLinhas = consulta($conn, 'fipe', "
    SELECT a.id, a.fipe_preco_id, a.preco, a.titulo, a.preco_texto_bruto,
           COALESCE(a.ano_final,a.ano_inicial) AS ano, f.preco AS preco_fipe
...
foreach ($fipeLinhas as &$linhaFipe) {
    ...
    if (!mercado_ano_comparavel_fipe($linhaFipe)) continue; // modelo antigo: FIPE nao e referencia (F0c)
    $comparaveisFipe++;
