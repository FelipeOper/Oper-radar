...
class ComparadorMercadoContractTest(unittest.TestCase):
    def test_api_e_interface_suportam_tres_escopos_bilaterais(self):
        ...
        # A tela do Comparador vive em ComparadorBlocos.jsx (layout da DEMO); as regras em comparadorModel.js.
        blocos = (ROOT / "app" / "src" / "ComparadorBlocos.jsx").read_text(encoding="utf-8")
        modelo = (ROOT / "app" / "src" / "comparadorModel.js").read_text(encoding="utf-8")
        ...
        self.assertIn("${prefixo}_ano", blocos)
        self.assertIn("Ano-modelo", blocos)
        self.assertIn("Cada lado combina marca, modelo ou marca + modelo", blocos)
        self.assertIn("AMOSTRA_MINIMA = 5", modelo)
        self.assertIn("<PageComparador ", app)
